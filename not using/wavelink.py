...
class Player(wavelink.Player):
    ...
    async def add_tracks1(self, ctx, tracks): #when using jek p, add_tracks1 will pick first in the order
        if not tracks:
            raise NoTracksFound
    ...
    async def button_select_callback(self, interaction):
        await interaction.send(interaction.message, ephemeral=False)
...
class Music(commands.Cog, wavelink.WavelinkMixin):
    ...
    async def button_callback(self, interaction):
        ...
        if cmd == "resume":
            ...
            if player.queue.is_empty:
                interaction.send("Queue is empty")
                return
            ...
            embed = discord.Embed(
                ...
                colour=interaction.author.colour,
                timestamp=dt.datetime.utcnow()
            )
            embed.set_author(name="Query Results")
            embed.set_footer(text=f"Requested by {interaction.author.display_name}", icon_url=interaction.author.avatar_url)
            embed.add_field(
            ...
            if player.queue.repeat_mode.value == 0:
                player.queue.set_repeat_mode("all")
                await interaction.send("now looping")
            else:
                player.queue.set_repeat_mode("none")
                await interaction.send(f"now not looping")
            return
        elif cmd == "bass":
            bass_preset = {2:10, 3:10, 4:10}
            for band, gain in bass_preset.items():
                player.eq_levels[band - 1] = gain / 10
                eq = wavelink.eqs.Equalizer(levels=[(i, gain) for i, gain in enumerate(player.eq_levels)])
                await player.set_eq(eq)
            await interaction.send("Bass Boosted.")
            return
        elif cmd == "no bass":
            bass_preset = {2:0, 3:0, 4:0}
            for band, gain in bass_preset.items():
                player.eq_levels[band - 1] = gain / 10
                eq = wavelink.eqs.Equalizer(levels=[(i, gain) for i, gain in enumerate(player.eq_levels)])
                await player.set_eq(eq)
            await interaction.send("Bass no more.")
            return
        await interaction.send(":magic_wand: :musical_note: :musical_note: **เพลงค้าง บอทไม่เล่น บอทหนี โปรดแจ้งอาการที่ jek feedback [อาการ]**")


    @commands.command(name="c", aliases=["b"])
    async def control_command(self, ctx):
        player = self.get_player(ctx)
        await ctx.send(
            f'**กำลังเล่น : ** {player.queue.current_track}',
            components=[[
                    self.bot.components_manager.add_callback(
                        Button(style=ButtonStyle.green, label="PLAY", custom_id="resume"),self.button_callback
                    ),
                    self.bot.components_manager.add_callback(
                        Button(style=ButtonStyle.grey, label="PAUSE", custom_id="pause"),self.button_callback
                    ),
                    self.bot.components_manager.add_callback(
                        Button(style=ButtonStyle.blue, label="SKIP", custom_id="skip"),self.button_callback
                    ),
                    self.bot.components_manager.add_callback(
                        Button(style=ButtonStyle.red, label="LEAVE", custom_id="leave"),self.button_callback
                    ),
                    self.bot.components_manager.add_callback(
                        Button(style=ButtonStyle.grey, label="QUEUE", custom_id="queue"),self.button_callback
                    ),
                ],
                [
                    self.bot.components_manager.add_callback(
                        Button(style=ButtonStyle.grey, label="LOOP", custom_id="loop"),self.button_callback
                    ),
                    self.bot.components_manager.add_callback(
                        Button(style=ButtonStyle.red, label="BASS", custom_id="bass"),self.button_callback
                    ),
                    self.bot.components_manager.add_callback(
                        Button(style=ButtonStyle.grey, label="NO BASS", custom_id="no bass"),self.button_callback
                    ),
                ]]
            )
    ...
    async def connect_command_error(self, ctx, exc):
        if isinstance(exc, AlreadyConnectedToChannel):
            await ctx.send("ว่าไง", delete_after=7)
    ...
    async def play_command(self, ctx, *, query: t.Optional[str]):
        ...
        if query is None:
            ...
            await player.set_pause(False)
            await ctx.send("เล่นต่องับ", delete_after=7)
    ...
    async def play2_command(self, ctx, *, query: t.Optional[str]):
        ...
        if query is None:
            ...
            await player.add_tracks1(ctx, await self.wavelink.get_tracks(query))
    ...
    async def repeat_command(self, ctx):
        ...
        if player.queue.repeat_mode.value == 0:
            player.queue.set_repeat_mode("all")
            await ctx.send(f"now looping {ctx.author.mention}")
        else:
            player.queue.set_repeat_mode("none")
            await ctx.send(f"now not looping {ctx.author.mention}")
        await ctx.message.delete()
    ...
    @commands.command(name="bass", aliases=["boost"])
    async def bassboost_command(self, ctx, *, level=10):
        player = self.get_player(ctx)
        bass_preset = {2:level, 3:level, 4:level}

        for band, gain in bass_preset.items():
            player.eq_levels[band - 1] = gain / 10
            eq = wavelink.eqs.Equalizer(levels=[(i, gain) for i, gain in enumerate(player.eq_levels)])
            await player.set_eq(eq)
        await ctx.send(f"Bass Boosted to level {level}.")
